...
//        method.invoke() //
        ...
        if(handler.getController().getClass().isAnnotationPresent(Security.class) ||
            ...
            if(handler.getMethod().isAnnotationPresent(Security.class)){
                ...
                String[] value = annotation.value();
                int strLen1=allUsers.length;//保存第一个数组长度
                int strLen2=value.length;//保存第二个数组长度
                allUsers= Arrays.copyOf(allUsers,strLen1+ strLen2);//扩容
                System.arraycopy(value, 0, allUsers, strLen1,strLen2 );//将第二个数组与第一个数组合并
            }
            if(allUsers != null && allUsers.length>0){
                //获取用户名，判断此用户名是否存在于允许列表
                String username = req.getParameter("name");
                Boolean check = Arrays.asList(allUsers).contains(username);
                if(!check){
                    req.setCharacterEncoding("UTF-8");
                    resp.setContentType("text/html;charset=utf-8");
                    System.out.println("当前用户无权限访问");
